            ...
            Command::DeclareFun {
                ...
                crate::concrete::Sort::Simple { identifier } => match identifier {
                    ...
                    crate::concrete::Identifier::Indexed { symbol, indices } => {
                        let indices_str = indices.iter().map(|index| index.to_string()).collect::<Vec<String>>().join(" ");
                        format!("({} {})", symbol.0, indices_str)
                    }
                },
                crate::concrete::Sort::Parameterized { identifier, parameters } => match identifier {
                    crate::concrete::Identifier::Simple { symbol } => symbol.0.clone(),
                    crate::concrete::Identifier::Indexed { symbol, indices } => {
                        let param_str = parameters.iter().map(|index| index.to_string()).collect::<Vec<String>>().join(" ");
                        let indices_str = indices.iter().map(|index| index.to_string()).collect::<Vec<String>>().join(" ");
